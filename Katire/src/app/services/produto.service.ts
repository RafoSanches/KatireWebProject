...
export class ProdutoService {
  ...
  buscarProdutos(termo: string): Observable<Produto[]> {
    return this.http.get<Produto[]>(`${this.api}/produtos?titulo_like=${termo}&ativo=true`);
  }
  ...
  recalcularMediaAvaliacao(avaliadoId: string): Observable<Usuario> {
    return new Observable(obs => {
      this.getAvaliacoesPorAvaliado(avaliadoId).subscribe(avs => {
        ...
        this.atualizarUsuario(avaliadoId, { avaliacoes: +media.toFixed(1), totalTrocas: avs.length }).subscribe(u => {
