...
@Component({
            ...
            <form [formGroup]="formAvaliacao" (ngSubmit)="enviarAvaliacao()">
              ...
              <div class="criterios-grid">
                <div class="field">
                  ...
                  <select formControlName="comunicacao">
                    <option value="">Selecione</option>
                    <option *ngFor="let n of [1,2,3,4,5]" [value]="n">{{ n }} / 5</option>
                  </select>
                </div>
                <div class="field">
                  <label>Produto conforme *</label>
                  <select formControlName="descricaoProduto">
                    <option value="">Selecione</option>
                    <option *ngFor="let n of [1,2,3,4,5]" [value]="n">{{ n }} / 5</option>
                  </select>
                </div>
                <div class="field">
                  ...
                  <select formControlName="pontualidade">
                    <option value="">Selecione</option>
                    <option *ngFor="let n of [1,2,3,4,5]" [value]="n">{{ n }} / 5</option>
                  </select>
                </div>
              </div>
              <span class="erro-campo" *ngIf="formAvaliacao.touched && (formAvaliacao.get('comunicacao')!.invalid || formAvaliacao.get('descricaoProduto')!.invalid || formAvaliacao.get('pontualidade')!.invalid)">
                Avalie os três critérios da troca.
              </span>
  ...
  styles: [`
    ...
    .criterios-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 0.65rem; margin: 1rem 0 0.35rem; }
    select, textarea {
      padding: 0.75rem 1rem; border: 1.5px solid #e5e7eb; border-radius: 10px;
      font-size: 0.95rem; outline: none; font-family: inherit; background: #fff;
      transition: border-color 0.2s;
    }
    textarea { resize: vertical; }
    select:focus, textarea:focus { border-color: #f97316; }
    ...
    @media (max-width: 560px) {
      .modal { max-height: 95vh; overflow-y: auto; margin: 0.75rem; }
...
export class PropostasComponent implements OnInit {
  ...
  constructor(
    ...
    this.formAvaliacao = this.fb.group({
      ...
      pontualidade: ['', Validators.required],
  ...
  enviarAvaliacao() {
    ...
    const propostaId = this.propostaAtual!.id!;
    ...
    const avaliacao: import('../../models/produto.model').Avaliacao = {
      ...
      comunicacao: Number(this.formAvaliacao.value.comunicacao),
      descricaoProduto: Number(this.formAvaliacao.value.descricaoProduto),
      pontualidade: Number(this.formAvaliacao.value.pontualidade),
      criadaEm: new Date().toISOString()
    };
    ...
    this.produtoService.criarAvaliacao(avaliacao).subscribe(() => {
      this.produtoService.recalcularMediaAvaliacao(this.avaliadoId).subscribe(() => {
        this.jaAvaliado[propostaId] = true;
        this.produtoService.criarNotificacao({
          usuarioId: this.avaliadoId,
          tipo: 'avaliacao',
          titulo: 'Nova avaliação recebida',
          mensagem: `${usuario?.nome ?? 'Um usuário'} avaliou sua troca com ${this.notaSelecionada} estrela${this.notaSelecionada === 1 ? '' : 's'}.`,
          link: '/app/perfil',
          lida: false,
          criadaEm: new Date().toISOString()
        }).subscribe();
