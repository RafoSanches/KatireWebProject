...
@Component({
      ...
      <div class="layout" *ngIf="usuario">
        ...
        <div class="sidebar">
          <div class="avatar-card">
            <img *ngIf="usuario.avatar; else avatarUsuario" [src]="usuario.avatar" [alt]="usuario.nome" class="avatar" />
            <ng-template #avatarUsuario><div class="avatar avatar-placeholder">{{ iniciais(usuario.nome) }}</div></ng-template>
            <h2>{{ usuario.nome }}</h2>
            <p>{{ usuario.email }}</p>
            <div class="stats">
          ...
          <div class="reputacao-card">
            ...
            <div class="lista-avaliacoes" *ngIf="avaliacoes.length > 0">
              <div class="card-avaliacao" *ngFor="let av of avaliacoes">
                <div class="av-header">
                  <img *ngIf="av.autorAvatar; else avatarAutor" [src]="av.autorAvatar" [alt]="av.autorNome" class="av-avatar" />
                  <ng-template #avatarAutor><span class="av-avatar av-placeholder">{{ iniciais(av.autorNome || 'Usuário') }}</span></ng-template>
                  <div class="av-info">
                ...
                <div class="av-criterios" *ngIf="av.comunicacao || av.descricaoProduto || av.pontualidade">
                  <span>Comunicação {{ av.comunicacao || av.nota }}/5</span>
                  <span>Produto {{ av.descricaoProduto || av.nota }}/5</span>
                  <span>Pontualidade {{ av.pontualidade || av.nota }}/5</span>
                </div>
              </div>
            </div>
          </div>
  ...
  styles: [`
    ...
    .avatar-placeholder { display:grid; place-items:center; background:#fff7ed; color:#ea580c; font-size:1.5rem; font-weight:800; }
    ...
    .av-placeholder { display:grid; place-items:center; background:#ffedd5; color:#c2410c; font-size:.62rem; font-weight:800; }
    ...
    .av-criterios { display:flex; flex-wrap:wrap; gap:.35rem; margin-top:.65rem; }
    .av-criterios span { padding:.25rem .45rem; border-radius:999px; background:#fff; color:#6b7280; font-size:.68rem; font-weight:700; }
